...
void Drive_Motor(float Vx,float Vy,float Vz)
{
	  ...
	  if (Car_Mode==Mec_Car) 
    {
		...
		else if (Car_Mode==Akm_Car) 
		{
			...
			Servo=SERVO_INIT + (Angle_Servo - 1.573f)*Ratio;
...
void Robot_Odometry(float *__Robot_Pos_X,float *__Robot_Pos_Y,float *__Robot_Pos_Z,float *__Robot_Pos)
{
	...
	//计算akm底盘的vx与vz
	float Chassis_Vx = (MOTOR_A.Encoder + MOTOR_B.Encoder)/2.0f;
	float Chassis_Vz = (MOTOR_B.Encoder - MOTOR_A.Encoder)/Wheel_spacing;
	//计算行程
