...
struct Struct_UART_Tx_Data uart3_tx_data;
struct Struct_UART_Rx_Data uart3_rx_data;
#define USART3_TX_BUF_SIZE sizeof(uart3_tx_data)
#define USART3_RX_BUF_SIZE sizeof(uart3_rx_data)
...
void data_transition(void)
{
	...
	Send_Data.Sensor_Str.Flag_Control_Enable = Flag_Stop;
  ...
  //数据校验位计算，模式1是发送数据校验
  	Send_Data.Sensor_Str.crc = Check_Sum(22,1);
	Send_Data.buffer[22]=Send_Data.Sensor_Str.crc;
	
	Send_Data.buffer[23]=Send_Data.Sensor_Str.Frame_Tail; //Frame_tail //帧尾
}
#endif
void data_transition()
{
	uart3_tx_data.Frame_Header = FRAME_HEADER; //Frame_header //帧头
	uart3_tx_data.Flag_Control_Enable = Flag_Stop;//0使能，1失能 //小车软件失能标志位

	if(Car_Mode == Akm_Car)
	{
		uart3_tx_data.Now_X_speed = ((MOTOR_A.Encoder+MOTOR_B.Encoder)/2)*1000; //The X-axis speed of the car //小车X轴速度
		uart3_tx_data.Now_Y_speed = 0;//The Y-axis speed of the car //小车Y轴速度
		uart3_tx_data.Now_Z_speed = ((MOTOR_B.Encoder-MOTOR_A.Encoder)/Wheel_spacing)*1000;//The Z-axis speed of the car //小车Z轴速度
	}
	else
	{
		uart3_tx_data.Now_X_speed = 0;
		uart3_tx_data.Now_Y_speed = 0;
		uart3_tx_data.Now_Z_speed = 0;
	}

	uart3_tx_data.Power_Voltage = Voltage*1000; //The battery voltage of the car //电池电压

	//The acceleration of the triaxial acceleration //加速度计三轴加速度
	uart3_tx_data.Mpu6050_Accelerometer_X_Data= accel[1]; //The accelerometer Y-axis is converted to the ros coordinate X axis //加速度计Y轴转换到ROS坐标X轴
	uart3_tx_data.Mpu6050_Accelerometer_Y_Data=-accel[0]; //The accelerometer X-axis is converted to the ros coordinate y axis //加速度计X轴转换到ROS坐标Y轴
	uart3_tx_data.Mpu6050_Accelerometer_Z_Data= accel[2]; //The accelerometer Z-axis is converted to the ros coordinate Z axis //加速度计Z轴转换到ROS坐标Z轴
	
	//The Angle velocity of the triaxial velocity //角速度计三轴角速度
	uart3_tx_data.Mpu6050_Gyroscope_X_Data= gyro[1]; //The Y-axis is converted to the ros coordinate X axis //角速度计Y轴转换到ROS坐标X轴
	uart3_tx_data.Mpu6050_Gyroscope_Y_Data=-gyro[0]; //The X-axis is converted to the ros coordinate y axis //角速度计X轴转换到ROS坐标Y轴
	
	if(Flag_Stop==0) 
		//If the motor control bit makes energy state, the z-axis velocity is sent normall
		uart3_tx_data.Mpu6050_Gyroscope_Z_Data=gyro[2];  //If the motor control bit makes energy state, the z-axis velocity is sent normall
	else  
		//If the robot is static (motor control dislocation), the z-axis is 0
...
{
  unsigned char i = 0;
  memcpy(uart3_tx_buffer, &uart3_tx_data, USART3_TX_BUF_SIZE);
	for(i=0; i<USART3_TX_BUF_SIZE; i++)
	{
		usart3_send(uart3_tx_buffer[i]);
	}	 
}
/**************************************************************************
...
int USART3_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART3, USART_IT_RXNE) != RESET) // Check if data is received //判断是否接收到数据
	{
		...
		// Fill the array with serial data
		// 串口数据填入数组
		uart3_rx_buffer[Count] = Usart_Receive;
		...
		if (Count == USART3_RX_BUF_SIZE) // Verify the length of the packet //验证数据包的长度
		{
			Count = 0;	
										   // Prepare for the serial port data to be refill into the array //为串口数据重新填入数组做准备
			// Copy the received data to the buffer
			memcpy(&uart3_rx_data,uart3_rx_buffer,USART3_RX_BUF_SIZE);
			
			if (uart3_rx_data.Frame_Tail == FRAME_TAIL) // Verify the frame tail of the packet //验证数据包的帧尾
			{
				// Data exclusionary or bit check calculation, mode 0 is sent data check
				// 数据异或位校验计算，模式0是发送数据校验
				if (uart3_rx_data.crc == Check_Sum(USART3_RX_BUF_SIZE-2, 0))
				{
					float Vz;
					// All modes flag position 0, USART3 control mode
					// 所有模式标志位置0，为Usart3控制模式
					...
					// Calculate the target speed of three axis from serial data, unit m/s
					// 从串口数据求三轴目标速度， 单位m/s
					Move_X = (float)uart3_rx_data.Set_X_speed / 1000.0f;
					Move_Y = 0.0f;
					Vz = (float)uart3_rx_data.Set_Z_speed / 1000.0f;
...
u8 Check_Sum(unsigned char Count_Number,unsigned char Mode)
{
	...
	{
	check_sum=check_sum^uart3_tx_buffer[k];
	}
	...
	for(k=0;k<Count_Number;k++)
	{
	check_sum=check_sum^uart3_rx_buffer[k];
	}
	return check_sum;
}
